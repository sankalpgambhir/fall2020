...
\begin{document}
    ...
    \question*{Determine the eigenvalues and eigenvectors of the 2 x 2 matrix $\sigmavector$.$\unitvectorn$ , where $\unitvectorn$ is a unit vector along the $(\theta, \phi)$ direction and $\sigmavector$ are the three Pauli matrices. This is basically the projection of the spin 1/2 operator (apart from $\frac{\hbar}{2}$) along the direction of the unit vector $\unitvectorn$. Do this in two ways: }

    \textbf{(a).} First by explicitly diagonalizing the matrix $\sigmavector$.$\unitvectorn$.
    \hrule

    The vector $\sigmavector$ = $(\sigma_{1}, \sigma_{2}, \sigma_{3})$, where the $\sigma_{i}$ matrices are -
    ...
    Now we need to figure out what $\unitvectorn$ is. The unit vector points along the $(\theta, \phi)$ direction. This is nothing but the unit vector $\hat{\mathbf{r}}$ in Polar co-ordinates.
    \begin{equation*}
        \unitvectorn = \boldsymbol{\hat{r}} = \textrm{cos}(\phi)\textrm{sin}(\theta)\mathbf{\hat{i}} + \textrm{sin}(\phi)\textrm{sin}(\theta)\mathbf{\hat{j}} + \textrm{cos}(\theta)\mathbf{\hat{k}}
    \end{equation*}
    ...
    If we apply consecutive rotation operators, we should be able to rotate this spinor into a general state, pointing in an arbitrary direction $\unitvectorn$, where $\unitvectorn$ points in the $(\theta, \phi)$ direction. \\
